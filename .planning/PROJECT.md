# Project: pickaladder

## Overview
A modern web application for managing pickleball ladders, leagues, and tournaments.

## Goal
Add GSD planning to the project to facilitate the implementation of new features and maintain code quality.

## Tech Stack
- **Backend:** Python (Flask/Django - to be confirmed by research)
- **Deployment:** Docker, Nginx
- **Database:** Firestore (based on `firestore.indexes.json`)
- **Frontend:** HTML/CSS (Existing styles)

## Preferences
- **Research Depth:** Domain research on pickleball ladder systems.
- **UI Style:** Preserve existing styles.
- **Workflow:** GSD (Research -> Strategy -> Execution).

## Context
This project is an existing codebase with features like user profiles, social/friend systems, groups/leagues, match recording, and an admin panel. It emphasizes AI-readiness and code quality.
