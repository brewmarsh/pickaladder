...
def _fetch_social_and_tournaments(db: Client, user_id: str) -> dict[str, Any]:
    ...
    # ⚡ Bolt Optimization:
    # What: Execute 8 independent database query functions concurrently using ThreadPoolExecutor.
    # Why: Previously, these functions were called sequentially to build the dictionary,
    #      causing a severe N+1 latency bottleneck as each awaited the previous one's network response.
    # Impact: Expected to reduce total latency for this aggregation block by ~4-8x.
