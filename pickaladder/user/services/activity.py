...
def get_dashboard_data(
    ...
    # 1. Fetch user and cached vanity stats
    user_data, vanity_metrics = _fetch_vanity_stats(db, user_id)

    # 2. Setup activity data placeholders
    total_matches = vanity_metrics.get("total_matches", 0)
    match_data = (
        ...
        if include_activity
            ...
            "streak_type": vanity_metrics.get("streak_type", "N/A"),
            "recent_opponents": [],
        }
    )
    ...
    # 4. Calculate Onboarding Progress using cached totals
...
def _fetch_vanity_stats(
    ...
    """Fetch user document and calculate vanity metrics using the cached lifetime stats."""
    ...
    # Primary Path: Fetch cached lifetime stats from subcollection
    stats_ref = (
        db.collection("users")
        .document(user_id)
        .collection("stats")
    ...
    try:
        ...
        if stats_snap.exists:
            # Use the backfilled/cached stats (Fast Path)
            vanity_metrics = extract_lifetime_vanity_metrics(stats_snap.to_dict() or {})
            return user_data, vanity_metrics
    except Exception:
        pass

    # Fallback Path: If cache is missing, use user-level legacy stats
    user_stats = user_data.get("stats") or {}
    vanity_metrics = calculate_vanity_metrics(user_stats)
    vanity_metrics.setdefault("current_streak", 0)
    vanity_metrics.setdefault("streak_type", "N/A")

    return user_data, vanity_metrics


def _fetch_recent_activity(db: Client, user_id: str) -> dict[str, Any]:
    """Fetch recent matches (limit 10) for activity feed and current engagement."""
