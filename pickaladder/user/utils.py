    ...
    for tournament in tournaments_query:
        ...
        for p in participants:
            ...
            if p_uid == ghost_ref.id:
                ...
                # If already a participant, we drop the ghost entry to deduplicate
            else:
...
class UserService:
    ...
    def get_past_tournaments(db: Client, user_id: str) -> list[dict[str, Any]]:
        ...
        past_tournaments.sort(key=lambda x: x.get("date") or datetime.datetime.min, reverse=True)
    ...
    def get_all_users(db: Client, exclude_user_id: str, limit: int = 20) -> list[dict[str, Any]]:
        """Fetch a list of users, excluding the current user."""
        users_query = (
            db.collection("users")
    ...
    @staticmethod
    def _process_h2h_match(data: dict[str, Any], user_id_1: str, user_id_2: str) -> tuple[int, int, int]:
    ...
    def get_user_matches(db: Client, user_id: str) -> list[Any]:
        ...
        user_ref = db.collection("users").document(user_id)
        matches_as_p1 = db.collection("matches").where(filter=firestore.FieldFilter("player1Ref", "==", user_ref)).stream()
        matches_as_p2 = db.collection("matches").where(filter=firestore.FieldFilter("player2Ref", "==", user_ref)).stream()
        matches_as_t1 = db.collection("matches").where(filter=firestore.FieldFilter("team1", "array_contains", user_ref)).stream()
        matches_as_t2 = db.collection("matches").where(filter=firestore.FieldFilter("team2", "array_contains", user_ref)).stream()
    ...
    def get_group_rankings(db: Client, user_id: str) -> list[dict[str, Any]]:
        ...
        user_ref = db.collection("users").document(user_id)
        group_rankings = []
        my_groups_query = db.collection("groups").where(filter=firestore.FieldFilter("members", "array_contains", user_ref)).stream()
    ...
    @staticmethod
    def _get_player_info(player_ref: DocumentReference, users_map: dict[str, Any]) -> dict[str, Any]:
    ...
    @staticmethod
    def _get_user_match_result(match_data: dict[str, Any], user_id: str, winner: str) -> str:
