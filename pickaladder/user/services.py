...
class UserService:
    ...
    def merge_users(db: Client, source_id: str, target_id: str) -> None:
        ...
        batch = db.batch()
        # Use the generic name from main, but logic from fix-branch
        ...
        for field in ["team1", "team2"]:
            matches = (
                ...
                .where(
                    filter=firestore.FieldFilter(
                        field, "array_contains", source_ref
                    )
                )
                .stream()
            )
            for match in matches:
                ...
                if field in m_data:
                    ...
                    # Advanced Logic: Ensure we reference the correct Team ID
                    # If the user merges, the composition of the team changes.
                    partner_ref = next(
                        (r for r in current_team if r != source_ref), None
                    )
                    if partner_ref:
                        new_team_id = TeamService.get_or_create_team(
                            db, target_ref.id, partner_ref.id
                        )
