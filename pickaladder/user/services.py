...
def _migrate_references(
    db: Client, batch: firestore.WriteBatch, source_uid: str, target_uid: str
) -> None:
    """Helper to update all Firestore references from a source user to a target user."""
    source_ref = db.collection("users").document(source_uid)
    target_ref = db.collection("users").document(target_uid)

    # 1. Update Matches (Singles)
    for field in ["player1Ref", "player2Ref"]:
        for match in (
            ...
            batch.update(match.reference, {field: target_ref})

    # 2. Update Matches (Doubles)
    for field in ["team1", "team2"]:
        for match in (
            ...
            batch.update(match.reference, {field: firestore.ArrayRemove([source_ref])})
            batch.update(match.reference, {field: firestore.ArrayUnion([target_ref])})

    # 3. Update Matches (Old Formats/Strings)
    for field in ["player1Id", "player2Id", "partnerId", "opponent2Id", "winnerId"]:
        for match in (
            db.collection("matches")
            .where(filter=firestore.FieldFilter(field, "==", source_uid))
            .stream()
        ):
            batch.update(match.reference, {field: target_uid})
    ...
    for group in groups_query.stream():
        batch.update(group.reference, {"members": firestore.ArrayRemove([source_ref])})
        batch.update(group.reference, {"members": firestore.ArrayUnion([target_ref])})

    # 5. Update Group Ownership
    groups_owned_query = db.collection("groups").where(
        filter=firestore.FieldFilter("ownerRef", "==", source_ref)
    )
    for group in groups_owned_query.stream():
        batch.update(group.reference, {"ownerRef": target_ref})

    # 6. Update Tournament Participants
    tournaments_query = db.collection("tournaments").where(
        filter=firestore.FieldFilter("participant_ids", "array_contains", source_uid)
    )
    for tournament in tournaments_query.stream():
        ...
        updated = False
        target_already_present = target_uid in data.get("participant_ids", [])
        ...
        for p in participants:
            ...
            if p_uid == source_uid:
                if target_already_present:
                    # Skip adding the source user if target is already there
                    updated = True
                    continue
                else:
                    if "userRef" in p:
        ...
        if updated:
            ...
            new_p_ids = [target_uid if pid == source_uid else pid for pid in p_ids]
            # Deduplicate p_ids
            new_p_ids = list(dict.fromkeys(new_p_ids))
            batch.update(
                tournament.reference,
                {"participants": new_participants, "participant_ids": new_p_ids},
            )

    # 7. Update Tournament Ownership
    tournaments_owned_query = db.collection("tournaments").where(
        filter=firestore.FieldFilter("ownerRef", "==", source_ref)
    )
    for tournament in tournaments_owned_query.stream():
        batch.update(tournament.reference, {"ownerRef": target_ref})

    tournaments_organizer_query = db.collection("tournaments").where(
        filter=firestore.FieldFilter("organizer_id", "==", source_uid)
    )
    for tournament in tournaments_organizer_query.stream():
        batch.update(tournament.reference, {"organizer_id": target_uid})
...
class UserService:
    ...
    def merge_users(source_uid: str, target_uid: str, db: Client | None = None) -> bool:
        ...
        logger.info(f"Merging user {source_uid} into {target_uid}")

        batch = db.batch()

        # Migrate all references
        _migrate_references(db, batch, source_uid, target_uid)

        # Archive/Disable source user in Firestore
        batch.update(source_ref, {"archived": True, "disabled": True})

        # Disable in Firebase Auth
        try:
            auth.update_user(source_uid, disabled=True)
        except Exception as e:
            logger.error(f"Failed to disable user in Firebase Auth: {e}")

        batch.commit()
        logger.info(f"Successfully merged {source_uid} into {target_uid}")
        return True
    ...
    def get_past_tournaments(db: Client, user_id: str) -> list[dict[str, Any]]:
        ...
        past_tournaments.sort(
            key=lambda x: x.get("date") or dt.datetime.min, reverse=True
        )
    ...
    def get_user_groups(db: Client, user_id: str) -> list[dict[str, Any]]:
        ...
        user_ref = db.collection("users").document(user_id)
        query = db.collection("groups").where(
    ...
    def calculate_stats(
        ...
        processed.sort(key=lambda x: x["date"] or dt.datetime.min, reverse=True)
...
def merge_ghost_user(db: Client, real_user_ref: Any, email: str) -> bool:
    ...
    try:
        ...
        batch = db.batch()
        _migrate_references(db, batch, ghost_doc.id, real_user_ref.id)
