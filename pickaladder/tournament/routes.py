...
def invite_group(tournament_id: str) -> Response:
    ...
    is_owner = t_data.get("organizer_id") == g.user["uid"] or (
        t_data.get("ownerRef") and cast(Any, t_data["ownerRef"]).id == g.user["uid"]
    )
    if not is_owner:
        flash("Unauthorized.", "danger")
        return cast(
    ...
    group_data = cast(dict[str, Any], group_doc.to_dict() or {})
    ...
    for member_ref in member_refs:
        if member_ref.id not in current_participant_ids:
            new_participants.append(
                {"userRef": member_ref, "status": "pending", "team_name": None}
            )
