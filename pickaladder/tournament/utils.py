...
def _record_match_result(standings: dict, id1: str, id2: str, s1: int, s2: int) -> None:
    ...
    if s1 > s2:
        ...
        standings[id1]["h2h"][id2] = standings[id1]["h2h"].get(id2, 0) + 1
        standings[id2]["h2h"][id1] = standings[id2]["h2h"].get(id1, 0)
    else:
        standings[id2]["wins"] += 1
        standings[id1]["losses"] += 1
        standings[id2]["h2h"][id1] = standings[id2]["h2h"].get(id1, 0) + 1
        standings[id1]["h2h"][id2] = standings[id1]["h2h"].get(id2, 0)
...
def sort_and_format_standings(
    ...
    def compare_participants(a: dict, b: dict) -> int:
        # 1. Wins
        if a["wins"] != b["wins"]:
...
def get_tournament_standings(
    ...
    matches = fetch_tournament_matches(db, tournament_id, pool_id=pool_id)
