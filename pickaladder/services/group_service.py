...
from google.cloud.firestore import Client, DocumentReference, DocumentSnapshot, FieldFilter
...
def get_group_details(
    ...
    # Singles Leaderboard
    leaderboard = _get_group_leaderboard(db, group_id, member_refs)
...
def _get_group_leaderboard(
    db: Client, group_id: str, member_refs: List[DocumentReference]
) -> List[Dict[str, Any]]:
    ...
    for player in current_leaderboard:
        ...
        for match_data in user_matches_map.get(user_id, []):
            ...
            if match_type == "doubles":
                ...
                p1_ref, p2_ref = match_data.get("player1Ref"), match_data.get(
                    "player2Ref"
                )
...
def create_email_invite(
    ...
    # 2. Check original if different
        ...
        query_orig = (
            users_ref.where(filter=FieldFilter("email", "==", original_email))
            .limit(1)
        )
...
def _get_recent_matches_and_players(
    ...
    for match_doc in recent_matches_docs:
        ...
        player_ids.add(_get_id(match_data, ["opponent2Id", "opponent2", "opponent2_id"]))
