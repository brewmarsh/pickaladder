...
def record_match() -> Any:
    ...
    if form.validate_on_submit():
        ...
        try:
            m_res = MatchService.record_match(db, submission, g.user)
            if request.is_json:
                return jsonify({"status": "success", "match_id": m_res.id}), 200
            ...
            return redirect(url_for("match.view_match_summary", match_id=m_res.id))
        except Exception as e:
            if request.is_json:
