...
class User(db.Model):
    ...
    friend_requests_sent = db.relationship('Friend', foreign_keys='Friend.user_id', backref='requester', lazy='dynamic', cascade='all, delete-orphan')
    # Friendships received by this user
    friend_requests_received = db.relationship('Friend', foreign_keys='Friend.friend_id', backref='requested', lazy='dynamic', cascade='all, delete-orphan')
...
class Friend(db.Model):
    ...
    friend_id = db.Column(UUID(as_uuid=True), db.ForeignKey('users.id', ondelete='CASCADE'), primary_key=True)
...
class Match(db.Model):
    ...
    id = db.Column(UUID(as_uuid=True), primary_key=True, server_default=func.uuid_generate_v4())
    player1_id = db.Column(UUID(as_uuid=True), db.ForeignKey('users.id'), nullable=False)
    player2_id = db.Column(UUID(as_uuid=True), db.ForeignKey('users.id'), nullable=False)
