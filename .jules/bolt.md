...
## 2024-05-20 - Parallelizing Fetch of Social and Tournament Data in Dashboard
**Learning:** In `pickaladder/user/services/dashboard.py`, `_fetch_social_and_tournaments` was executing 8 independent Firestore queries (`get_user_friends`, `get_user_pending_requests`, `get_group_rankings`, `get_top_groups`, `TeamService.get_top_teams`, `get_pending_tournament_invites`, `get_active_tournaments`, `get_past_tournaments`) sequentially. This sequential fetch created an N+1 latency bottleneck where each query waited for the prior one to finish, directly slowing down the user dashboard load time.
**Action:** Replaced sequential fetches with concurrent execution using `concurrent.futures.ThreadPoolExecutor(max_workers=8)`. This reduces the total latency for this aggregation block from the sum of all request times to approximately the longest single request time.
