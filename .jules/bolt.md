...
## 2024-05-14 - Firestore N+1 Query Resolution using db.get_all()
**Learning:** Sequential `DocumentReference.get()` or `db.collection().document().get()` calls in a loop cause significant N+1 network overhead and latency in the Python `google-cloud-firestore` SDK.
**Action:** Always batch related document reads by collecting unique IDs, constructing a list of `DocumentReference` objects, and utilizing `db.get_all(refs)` to fetch them in a single network round-trip. Map the results manually to preserve associations.
