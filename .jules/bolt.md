...
## 2026-09-08 - [Batching independent Firestore Document References]\n**Learning:** In the Python `google-cloud-firestore` SDK, `db.get_all()` accepts an iterable of `DocumentReference` objects that can belong to different collections simultaneously. This makes it highly effective for batching multiple independent document reads (e.g., fetching a user, matches, and a group at the same time) into a single network request to drastically reduce database latency.\n**Action:** When route handlers perform multiple `.get()` operations for distinct context documents, consolidate them into a single `db.get_all()` request using an aggregated list of references, and map the results back using `doc.reference.path`.
