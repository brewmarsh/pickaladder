...
## 2025-02-21 - Deterministic Ordering with ThreadPoolExecutor
**Learning:** When using `concurrent.futures.ThreadPoolExecutor` to execute sequential database calls concurrently, iterating over `concurrent.futures.as_completed(futures)` appends results in the non-deterministic order that threads complete. This causes unpredictable array sorting and flaky tests.
**Action:** Always use `executor.map(function, iterable)` instead of `as_completed` when the exact list order must be preserved, as `map` inherently yields results in the same order as the input iterable.
