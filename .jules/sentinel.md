...
## 2024-05-18 - [DOM XSS in Match Recording]
**Vulnerability:** A DOM-based XSS vulnerability was found in `pickaladder/static/js/match_recording.js`. When dynamically building a roster form, user-controlled input (`member.name`) fetched from the server was interpolated directly into an HTML string and assigned to `rosterList.innerHTML`.
**Learning:** Even when fetching data directly from your own API (like a team roster), any field that was originally user-generated (like a username or team name) must be treated as untrusted data when rendering on the client side.
**Prevention:** Avoid string interpolation with `innerHTML` entirely when handling user data. Use safe DOM manipulation APIs like `document.createElement()`, `element.className = ...`, and `element.textContent = ...` which inherently escape special characters and prevent script execution.
