...
## 2025-02-24 - [Fix Authorization Bypass in Group Join Route]
**Vulnerability:** Insecure Direct Object Reference (IDOR) / authorization bypass in the `pickaladder/group/routes/membership.py` `join_group` route (`/<string:group_id>/join`). The route processed POST requests to add a user to a group without verifying the group's `join_policy` (e.g., OPEN vs. REQUEST/INVITE). This allowed an attacker to bypass the UI flow and forcefully join a restricted or private group by directly calling the endpoint.
**Learning:** Action routes that modify group membership or grant access must explicitly re-verify the governing authorization attributes (like `join_policy`) on the server side. UI-level hiding of the "Join" button for closed groups does not protect the underlying API endpoint.
**Prevention:** Always fetch the target resource (e.g., the group document) in the route handler and strictly validate its access control fields (`join_policy == "OPEN"`) before executing state-changing logic like `firestore.ArrayUnion`.
