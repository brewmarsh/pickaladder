...
## 2024-09-04 - [CRITICAL] Fix authorization bypass in group join route
**Vulnerability:** The `/join` endpoint for groups did not verify the group's `join_policy`. It directly updated the group's `members` array with the requesting user's ID, allowing any authenticated user to join any group (including private/invite-only ones) via a direct POST request.
**Learning:** UI-level restrictions (e.g., hiding the "Join" button) are insufficient. Direct API endpoints that modify state must independently verify the target object's access control policies (e.g., `join_policy` == 'OPEN').
**Prevention:** Always fetch the target entity and verify its state/policy permissions before performing mutations on behalf of a user.
