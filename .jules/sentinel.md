...
## 2024-05-18 - [Fix Authorization Bypass in Group Join Route]
**Vulnerability:** The `/join` route for groups in `pickaladder/group/routes/membership.py` allowed any user to join any group by directly hitting the endpoint with a POST request, bypassing the group's `join_policy` (e.g., OPEN, REQUEST, INVITE). This is a form of Insecure Direct Object Reference (IDOR) / authorization bypass.
**Learning:** UI-level protections (hiding the "Join" button for closed groups) are insufficient. All access control and business logic rules (like `join_policy`) must be enforced server-side on the action routes themselves.
**Prevention:** Always fetch the target resource in action routes and explicitly verify the user's authorization and the resource's state (e.g., `group.join_policy == "OPEN"`) before allowing the action to proceed.
