...
## 2023-10-24 - [IDOR in Group Join Endpoint]
**Vulnerability:** The `/group/<group_id>/join` endpoint allowed any authenticated user to join any group directly via a POST request, bypassing the group's configured `join_policy` (e.g., 'REQUEST', 'INVITE'). This is an Insecure Direct Object Reference (IDOR) / Broken Access Control issue, as the route did not explicitly check the group's policy server-side.
**Learning:** UI-level protections (like hiding the "Join" button for closed groups in the templates) are insufficient. Attackers can manually craft POST requests to the endpoint to bypass frontend restrictions.
**Prevention:** Always retrieve the target object's document and enforce authorization policies (like checking `join_policy == "OPEN"`) explicitly in the route handler before performing state-changing operations like adding a user to a group.
