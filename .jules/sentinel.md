...
## 2024-05-20 - [Fix IDOR in Group Join Route]
**Vulnerability:** Insecure Direct Object Reference (IDOR) and authorization bypass in the `pickaladder/group/routes/membership.py` `join_group` route (`/<group_id>/join`). The route lacked verification of the group's `join_policy` before processing a POST request to add a user to the group's `members` array.
**Learning:** Action routes (POST/PUT/DELETE) that modify resources like group membership must explicitly verify server-side constraints (like `join_policy`) to prevent authorization bypass. Relying solely on the UI to hide the "Join" button for non-"OPEN" groups is insufficient, as attackers can bypass the UI and send POST requests directly to the endpoint.
**Prevention:** Always fetch the target resource in the route handler and verify that the resource's configuration (e.g., `join_policy == "OPEN"`) permits the requested action before executing the database update.
