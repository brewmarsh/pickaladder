...
## 2024-05-20 - Fix DOM XSS in Toast Notifications
**Vulnerability:** Found a DOM-based Cross-Site Scripting (XSS) vulnerability in `pickaladder/static/js/main.js`. The `showToast` and `updateToast` functions were interpolating user-controlled data directly into HTML string templates or via `innerHTML`.
**Learning:** Client-side notification systems are common targets for XSS if they blindly trust the payload being displayed. Even simple messaging systems must treat their input as untrusted HTML unless it's strictly enforced otherwise.
**Prevention:** Always use safe DOM API properties like `textContent` when displaying text data in JavaScript. Do not use string interpolation with `innerHTML` or `insertAdjacentHTML` unless the data has been strongly sanitized first (e.g. DOMPurify or custom escape functions).
