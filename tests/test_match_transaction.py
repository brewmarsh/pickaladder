...
class MatchTransactionTestCase(unittest.TestCase):
    ...
    def test_record_match_atomic_singles(self) -> None:
                ...
                if name == "users":
                    ...
                    def doc_ref_collection_side_effect(sub_name):
                        ...
                        def sub_col_document_side_effect(sub_doc_id):
                            return create_mock_ref(
                                f"{doc_path}/{sub_name}/{sub_doc_id}"
                            )
        ...
        p1_snap = create_mock_snap(
            p1_ref, {"stats": {"wins": 5, "losses": 2, "elo": 1200.0}}
        )
        p2_snap = create_mock_snap(
            p2_ref, {"stats": {"wins": 3, "losses": 4, "elo": 1100.0}}
        )
        l1_snap = create_mock_snap(
            l1_ref, {"total_matches": 7, "wins": 5, "losses": 2, "current_streak": 2}
        )
        l2_snap = create_mock_snap(
            l2_ref, {"total_matches": 7, "wins": 3, "losses": 4, "current_streak": -1}
        )
        ...
        updates = {
            call[0][0].path: call[0][1] for call in transaction.update.call_args_list
        }
        ...
        sets = {
            call[0][0].path: call[0][1]
            for call in transaction.set.call_args_list
            if hasattr(call[0][0], "path")
        }
    ...
    def test_record_match_atomic_doubles(self) -> None:
        ...
        t1_snap = create_mock_snap(
            t1_ref, {"stats": {"wins": 10, "losses": 10, "elo": 1500.0}}
        )
        t2_snap = create_mock_snap(
            t2_ref, {"stats": {"wins": 20, "losses": 5, "elo": 1500.0}}
        )

        u_stats_snaps = [
            create_mock_snap(
                create_mock_ref(f"{r.path}/stats/lifetime"),
