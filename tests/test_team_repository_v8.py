...
def test_get_team_by_members_pairing_found(mock_db):
    # Setup mock
    mock_doc = MagicMock()
    ...
    team = TeamRepository.get_team_by_members(mock_db, ["u1", "u2"], team_type="pairing")
    
    assert team is not None
    assert team["id"] == "team_id"
    assert team["type"] == "pairing"

def test_get_team_by_members_legacy_found(mock_db):
...
def test_get_team_by_members_not_found(mock_db):
    mock_query = mock_db.collection.return_value.where.return_value
...
def test_create_named_team(mock_db):
    mock_doc_ref = MagicMock()
    mock_doc_ref.id = "new_named_team"
    mock_db.collection.return_value.document.return_value = mock_doc_ref
    
    team_id = TeamRepository.create_named_team(mock_db, "The Smashers", "creator_id", ["u1", "u2", "u3"])
...
def test_get_user_named_teams(mock_db):
    mock_doc = MagicMock()
    mock_doc.to_dict.return_value = {"type": "named", "name": "Team A", "member_ids": ["u1", "u2"]}
    mock_doc.id = "team_a"
