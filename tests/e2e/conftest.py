...
# Patch DocumentSnapshot for special field paths
...
class MockTransaction(Transaction):
    """Mock for firestore.Transaction with explicit lifecycle methods."""
...
def app_server(mock_db: EnhancedMockFirestore, mock_auth: MockAuthService) -> Generator[str, None, None]:
    ...
    patches = [
        ...
        patch.object(firebase_admin.firestore, "client", return_value=mock_db),
        patch.object(firebase_admin.auth, "create_user", side_effect=mock_auth.create_user),
        patch.object(firebase_admin.auth, "verify_id_token", side_effect=mock_auth.verify_id_token),
        patch.object(firebase_admin.auth, "get_user", side_effect=mock_auth.get_user),
        patch.object(firebase_admin.auth, "generate_email_verification_link", side_effect=mock_auth.generate_email_verification_link),
        patch.object(firebase_admin.firestore, "SERVER_TIMESTAMP", "2023-01-01T00:00:00"),
        patch.object(firebase_admin.firestore, "ArrayUnion", side_effect=mock_array_union),
        patch.object(firebase_admin.firestore, "ArrayRemove", side_effect=mock_array_remove),
        patch.object(firebase_admin.firestore, "FieldFilter", MockFieldFilter),
        patch.object(firebase_admin.firestore, "transactional", side_effect=lambda x: x),
        patch.object(google.cloud.firestore, "transactional", side_effect=lambda x: x),
    ]
                ...
                return {
                    signInWithEmailAndPassword: function(email, password) {
                        return Promise.resolve({
                            user: { getIdToken: () => Promise.resolve("token_" + email.split('@')[0]) }
                        });
                    },
