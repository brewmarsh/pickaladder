...
def test_user_journey(app_server: str, page_with_firebase: Page, mock_db: Any) -> None:
    ...
    # Verify redirect to Summary Page (Logic from Main)
    expect(page).to_have_url(re.compile(r".*/match/summary/.*"), timeout=15000)
    expect(page.locator("h1")).to_contain_text("Match Summary")
    # Verify flash message using inclusive locator (Logic from Feature Branch)
    expect(page.locator(".alert-success, .toast.alert-success, .toast-body").first).to_contain_text(
    ...
    expect(page.locator(".alert-success, .toast.alert-success, .toast-body").first).to_contain_text("Match deleted successfully")
