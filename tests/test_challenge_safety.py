...
def test_issue_challenge_expiration(mock_datetime, mock_db):
    ...
    mock_datetime.side_effect = lambda *args, **kw: datetime(*args, **kw) # Allow normal datetime creation
    
    # We need to mock the transaction and the collection
    mock_tx = MagicMock()
    mock_db.transaction.return_value = mock_tx
    
    # Capturing the data sent to transaction.set
    # This is tricky with @firestore.transactional
    # Let's mock the internal transactional function if possible or just check the call
    
    with patch("pickaladder.match.services.challenge_service.firestore") as mock_firestore:
...
if __name__ == "__main__":
    ...
    class TestChallengeSafety(unittest.TestCase):
        ...
        def test_active_limit(self, mock_firestore, mock_social):
            ...
            mock_query = MagicMock()
            self.db.collection.return_value.where.return_value.where.return_value = mock_query
            mock_query.get.return_value = [MagicMock(), MagicMock(), MagicMock()]
            
            with self.assertRaisesRegex(ValueError, "You already have 3 active challenges"):
