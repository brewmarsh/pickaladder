...
def test_submit_feedback(mock_db):
    ...
    with patch("pickaladder.services.feedback_service.datetime") as mock_datetime:
        ...
        feedback_id = FeedbackService.submit_feedback(mock_db, user_id, feedback_type, message)
...
def test_update_feedback_status(mock_db):
    ...
    with patch("pickaladder.services.notification_service.NotificationService.send_to_user") as mock_notify:
        success = FeedbackService.update_feedback_status(mock_db, feedback_id, status, admin_id)
