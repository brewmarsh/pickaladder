...
class UserRoutesFirebaseTestCase(unittest.TestCase):
    ...
    def setUp(self) -> None:
        """Set up the test case using isolated patches."""
        self.mock_db = MockFirestore()
        self.mock_auth_service = MagicMock()
        # Define EmailAlreadyExistsError to allow catching it in routes
    ...
    def test_api_dashboard_returns_group_match_flag(self) -> None:
        """Test response includes indicator for group matches."""
        self._set_session_user()
        self._setup_dashboard_mocks()
        match_data = {
            "matchType": "singles",
            ...
            "player1Score": 11,
            "player2Score": 5,
            "matchDate": datetime.datetime.now(datetime.timezone.utc),
            "groupId": "group123",
        }
    ...
    def test_view_user_includes_doubles_and_processes_matches(self, mock_render_template: MagicMock) -> None:
        """Test that view_user fetches and processes matches into the context."""
        self._set_session_user()
        self.mock_db.collection("users").document(MOCK_PROFILE_USER_ID).set({
            "username": "profile_user",
            "stats": {"wins": 10, "losses": 5},
        })
        self.mock_db.collection("users").document("opponent_id").set({"username": "opponent"})
        ...
        # Verify the "user_result" logic from fix branch
        self.assertEqual(kwargs["matches"][0]["user_result"], "win")
