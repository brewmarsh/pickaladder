...
class PWATestCase(unittest.TestCase):
    ...
    def test_manifest_served(self, mock_firestore: MagicMock, mock_init: MagicMock):
    ...
    def test_service_worker_served_from_root(
        ...
        self.assertEqual(response.status_code, 200)
        # Flask might send it as application/javascript or text/javascript depending on the system
    ...
    def test_layout_contains_pwa_elements(
        ...
        # But we can just test if the template exists and has the content if we want to be safe.
        # Or just use the test client on a simple route.
