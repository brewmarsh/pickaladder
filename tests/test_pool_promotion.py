...
def test_promote_pools_to_bracket(mock_db):
    ...
    with patch("pickaladder.tournament.utils.get_tournament_standings") as mock_stands:
        ...
        mock_stands.side_effect = [
            [{"id": "p1", "wins": 3}, {"id": "p2", "wins": 2}, {"id": "p3", "wins": 1}, {"id": "p4", "wins": 0}], # Pool A
            [{"id": "p5", "wins": 3}, {"id": "p6", "wins": 2}, {"id": "p7", "wins": 1}, {"id": "p8", "wins": 0}]  # Pool B
        ]
        ...
        with patch("pickaladder.tournament.services.generator.TournamentGenerator.generate_single_elimination") as mock_gen:
            mock_gen.return_value = [{"match": 1}]
            
            with patch("pickaladder.tournament.services.tournament_service.TournamentService.save_pairings") as mock_save:
                mock_save.return_value = 1
                
                res = TournamentService.promote_pools_to_bracket(t_id, uid, 2, db=mock_db)
