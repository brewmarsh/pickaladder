...
class TournamentRoutesFirebaseTestCase(unittest.TestCase):
    ...
    def test_create_tournament(self) -> None:
        ...
        # Use relaxed assertion from fix branch to avoid whitespace issues
    ...
    def test_edit_tournament_ongoing(self) -> None:
        """Test that match type cannot be changed if matches have started."""
        self._set_session_user(is_admin=True)
        ...
        response = self.client.post(
            ...
            data={
                "name": "Updated Name",
                "match_type": "doubles", # Attempt to change
                "start_date": "2024-07-01",
            },
            follow_redirects=True,
        )
        ...
        self.assertEqual(response.status_code, 200)
        data = self.mock_db.collection("tournaments").document(tournament_id).get().to_dict()
        self.assertEqual(data["matchType"], "singles") # Integrity preserved
    ...
    def test_delete_tournament(self) -> None:
        ...
        tournament_id = "test_tournament_id"
        self.mock_db.collection("tournaments").document(tournament_id).set({"name": "Bye"})
